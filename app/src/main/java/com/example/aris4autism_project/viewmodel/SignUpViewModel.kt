...
//                }
            ...
            password.length<6 || !password.matches(".*[A-Z].*".toRegex()) || !password.matches(".*[@#\$%^&+=].*".toRegex())->{
                ...
                fullname.isNotEmpty() && mobile.isNotEmpty() && email.isNotEmpty() && gender.isNotEmpty() && dob.isNotEmpty() && password.isNotEmpty() && confirmpassword.isNotEmpty()->{
                   signUpResult.value="Valid Creadentials*"
                 }
            }
        }
