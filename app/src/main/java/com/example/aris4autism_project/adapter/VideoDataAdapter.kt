...
//        var youtubePlayer=binding.youtubePlayer
    ...
    override fun onBindViewHolder(holder: viewBindData, position: Int)
    {
        ...
        Glide.with(holder.itemView.context)
            .load(slist.learners.get(position).thumbnailUrl)
            .into(holder.imgThumbnail)
        var videoUrlPass:String = " "

        holder.idConstraintClass.setTag("tagVideoUrl")
        if(slist.learners.get(position).videoLink==null)
        {
            holder.idConstraintClass.setTag(R.id.tagVideoUrl,slist.learners.get(position).videoUrl)
        }

        if(slist.learners.get(position).videoUrl==null)
        {
            holder.idConstraintClass.setTag(R.id.tagVideoUrl,slist.learners.get(position).videoLink)
        }
...
//                    holder.itemView.context.startActivity(intent)
                    bundle.putString("videouri",slist.learners.get(position).videoLink.toString())
...
//                }
            ...
            holder.txTitle.text = slist.learners.get(position).title.toString()

    }

    override fun getItemCount(): Int {
       return slist.learners.size
    }
